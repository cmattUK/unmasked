...
export default function Hero() {
  const { ref:unmaskedRef, inView:elementIsVisible} = useInView();
  return (
    <Element className="heroSection" id="Work">
      <section className="hero">
        <div className='relative lg:pt-8 overflow-hidden lg:h-[600px] xl:h-screen'>
          <div className="bg-heroback bg-cover bg-center h-screen z-10 lg:py-auto lg:h-[600px] xl:h-screen lg:-mt-4">
            <div className="mx-auto lg:h-[600px]">
              <div className="mx-auto md:grid  md:grid-cols-1 lg:mx-0 lg:max-w-none lg:grid-cols-2 bg-herobacksm bg-cover bg-left-bottom lg:bg-none min-h-screen  xl:h-screen">
                  <Image src={portrait} className='w-full hidden lg:block lg:pt-16 xl:pt-0 pt-0'  alt="A close-up image of Garry Bell staring intently at the audience"  priority/>
                <div  className=' lg:pt-4 flex h-full lg:h-[600px] xl:h-full justify-start items-center md:px-8 px-4'>
                  <div  className={elementIsVisible ? 'ease-in-out duration-700 translate-y-0 opacity-1 absolute lg:relative bottom-0 pb-8 md:pb-0 mb-8 z-30' : '-translate-y-20 opacity-0'}>
                    ...
                    <a ref={unmaskedRef} className="mt-4 px-4 py-2 bg-none  border-white border-2  text-white hover:border-[#dacf89] hover:bg-[#dacf89] active:bg-[#9f9761] hover:text-black inline-block text-center hover:cursor-pointer uppercase text-normal md:text-lg font-bold"  target="_blank" rel="noopener noreferrer" >Get the book</a>
                    </p>
                  </div>
                </div>
              </div>
